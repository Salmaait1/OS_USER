...
void createTable()
{
    // Chaque joueur reçoit 3 cartes, la carte coupable est le 13ème
    int i, j, c;
    for (i = 0; i < 4; i++)
        for (j = 0; j < 8; j++)
            tableCartes[i][j] = 0;
        ...
        if (fsmServer == 0)
        {
            switch (buffer[0])
            {
                case 'C':
                    ...
                    // fsmServer == 0 : attendre les connexions de tous les joueurs
                    ...
                    // Envoyer un message personnel pour communiquer son id
                    sprintf(reply, "I %d", id);
                    sendMessageToClient(tcpClients[id].ipAddress,
                                        tcpClients[id].port,
                                        reply);

                    // Envoyer un message broadcast pour communiquer la liste des joueurs
                    ...
                    if (nbClients == 4)
                    {
                        // Pour chaque joueur, envoyer ses 3 cartes et sa ligne dans tableCartes
                        for (i = 0; i < 4; i++)
                        {
                            ...
                            {
                                char temp[10];
                                sprintf(temp, " %d", tableCartes[i][j]);
                                strcat(reply, temp);
                            }
                            sendMessageToClient(tcpClients[i].ipAddress, tcpClients[i].port, reply);
                        }
                        // Envoyer un message à tout le monde pour définir le joueur courant = 0
                        sprintf(reply, "M 0");
                        broadcastMessage(reply);
                        fsmServer = 1;
                    }
        ...
        else if (fsmServer == 1)
        {
            switch (buffer[0])
            {
                case 'G':  // Accusation
                {
                    ...
                    if (guess == deck[12])  // La carte coupable est la carte d'indice 12
                    {
                    ...
                    {
                        sprintf(reply, "LOSS %s", tcpClients[pid].name);
                        broadcastMessage(reply);
                ...
                break;
                case 'O':  // Enquête de type "r" simplifiée
                {
                    int pid, objet;
                ...
                break;
                case 'S':  // Enquête de type "z"
                {
                    int pid, target, objet;
                    sscanf(buffer, "S %d %d %d", &pid, &target, &objet);
                    int count = tableCartes[target][objet];
                    sprintf(reply, "V %d %d", objet, count);
                    // Envoyer la réponse au joueur demandeur
                    sendMessageToClient(tcpClients[pid].ipAddress, tcpClients[pid].port, reply);
                }
                break;
                default:
